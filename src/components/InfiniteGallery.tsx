...
function GalleryScene({
  ...
  useEffect(() => {
    const galleryContainer = document.getElementById('velvet-gallery');
    const canvas = galleryContainer?.querySelector('canvas');
    if (canvas) {
  ...
  useFrame((state, delta) => {
    if (autoPlay) {
      ...
      const ramp = Math.min(idleSeconds / 5, 4);
      const autoSpeed = 0.3 + ramp * 0.8;
...
export default function InfiniteGallery({
  ...
  if (!webglSupported) {
    return (
      <div className={`${className} flex items-center justify-center bg-[#111]`} style={style}>
        <p className="text-gray-400">3D Gallery requires WebGL support</p>
      </div>
    );
  }

  return (
    <div id="velvet-gallery" className={className} style={style}>
      <Canvas
